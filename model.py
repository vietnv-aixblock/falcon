...
if torch.cuda.is_available():
    ...
    _model = pipeline(
        "text-generation",
        model="tiiuae/Falcon3-1B-Instruct", #tiiuae/falcon-7b", #"tiiuae/Falcon3-1B-Base", tiiuae/Falcon3-3B-Instruct tiiuae/Falcon3-7B-Instruct
        torch_dtype=dtype, 
    ...
    _model = pipeline(
        "text-generation",
        model="tiiuae/Falcon3-1B-Instruct", #tiiuae/Falcon3-1B-Base", tiiuae/Falcon3-3B-Instruct tiiuae/Falcon3-7B-Instruct
        device_map="cpu",
        max_new_tokens=256,
        token = "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"
    )
...
class MyModel(AIxBlockMLBase):
    ...
    def action(self, project, command, collection, **kwargs):
        ...
        if command.lower() == "train":
                ...
                model_id = kwargs.get("model_id", "tiiuae/Falcon3-7B-Instruct")  #"tiiuae/falcon-7b" "bigscience/bloomz-1b7" `zanchat/falcon-1b` `appvoid/llama-3-1b` meta-llama/Llama-3.2-3B` `mistralai/Mistral-7B-v0.1` `bigscience/bloomz-1b7` `Qwen/Qwen2-1.5B`
                ...
                def func_train_model(clone_dir, project_id, token, checkpoint_version, checkpoint_id, dataset_version, dataset_id,model_id,world_size,rank,master_add,master_port,prompt, json_file):
                    ...
                    if framework == "huggingface":
                        ...
                        if world_size > 1:
                            ...
                            if model_id == None:
                                model_id = "tiiuae/Falcon3-1B-Instruct" # or  `appvoid/llama-3-1b` tiiuae/falcon-7b` `mistralai/Mistral-7B-v0.1` `bigscience/bloomz-1b7` `Qwen/Qwen2-1.5B`
